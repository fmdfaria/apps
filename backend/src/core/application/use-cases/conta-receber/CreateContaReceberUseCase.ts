...
interface CreateContaReceberRequest {
  empresaId: string;
...
export class CreateContaReceberUseCase {
  ...
  async execute(data: CreateContaReceberRequest): Promise<ContaReceber> {
    // Validações
    ...
    if (categoria.tipo !== 'RECEITA') {
      throw new Error('Categoria deve ser do tipo RECEITA');
    }
    ...
    return this.contasReceberRepository.create(conta);
  }
}