...
export function AppScreen({ className, contentClassName, edges, children, ...props }: AppScreenProps) {
  const safeEdges = edges ?? ['top'];

  return (
    <SafeAreaView className={cn('flex-1 bg-surface-background', className)} edges={safeEdges}>
      ...
      <View className={cn('flex-1 px-4 py-0', contentClassName)} {...props}>
        {children}
      </View>
    </SafeAreaView>
  );
